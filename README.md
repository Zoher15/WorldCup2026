...
- **Match cards**: one unified scoreboard for every state. Both flags, team
  ...
  `LIVE 67'`, `FULL TIME`). A card that's **open to predict** "goes live" with a
  soft ombre of the two flags' colours (home on the left half, away on the
  right) and active +/− steppers; non-open cards stay plain with greyed steppers.
  When locked, the card reads as a scoreboard: your pick, the result, and points.
...
- ✅ Prediction flow: a day's games all open the day before (00:00 UTC+14, "anywhere on earth") and lock at each kickoff, with live countdowns, shared across groups
- ✅ Per-player profile pages (`/g/[code]/p/[userId]`): past picks revealed, upcoming kept private (entered-or-not only)
